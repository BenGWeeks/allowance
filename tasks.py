...
async def resolve_lightning_address(lightning_address: str) -> Tuple[str, Dict[str, Any]]:
    """
...
async def get_invoice_from_lnurl(callback_url: str, amount_msats: int, memo: str = "") -> str:
    """
...
async def execute_lightning_address_payment(allowance: Allowance) -> bool:
    ...
    try:
        ...
        callback_url, lnurl_data = await resolve_lightning_address(allowance.lightning_address)
        ...
        max_sendable = lnurl_data.get('maxSendable', 100000000000)  # Default 100k sats max
        
        if amount_msats < min_sendable:
            raise Exception(f"Amount {allowance.amount} sats is below minimum {min_sendable // 1000} sats")
        if amount_msats > max_sendable:
            raise Exception(f"Amount {allowance.amount} sats exceeds maximum {max_sendable // 1000} sats")
        ...
        logger.error(f"❌ Error executing payment for allowance {allowance.name}: {str(e)}")
        return False
