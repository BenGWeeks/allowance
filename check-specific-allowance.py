            ...
            for a in allowances:
                if a['id'] == '3PTZjiEouMEybkxZnerqQm':
                    ...
                    if a.get('start_datetime'):
                        start = datetime.fromisoformat(a['start_datetime'].replace('Z', '+00:00'))
                        if now < start:
                            print(f"  ⏳ Status: Not started yet (starts in {int((start - now).total_seconds()/60)} minutes)")
                        else:
                            print(f"  ✅ Status: Started {int((now - start).total_seconds()/60)} minutes ago")
                    else:
                        print(f"  ✅ Status: No start time set (runs immediately)")

                    if a.get('end_datetime'):
                        end = datetime.fromisoformat(a['end_datetime'].replace('Z', '+00:00'))
                        if now > end:
                            print(f"  🛑 Status: Expired {int((now - end).total_seconds()/60)} minutes ago")
                        else:
                            print(f"  ⏰ Status: Ends in {int((end - now).total_seconds()/60)} minutes")
                    else:
